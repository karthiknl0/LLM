...
from app.core.config import SKILLS_DIR, WORKSPACE_DIR

from app.core.project import active_project_folder
TIMEOUT_SECONDS = 60
...
def run_python(code: str) -> str:
    ...
    new_files = sorted(
        {p.name for p in root.iterdir()} - before
    )
    if new_files:
...
def _resolve_cwd(cwd: str):
    """A working directory inside the selected project, or None."""
...
def run_command(command: str, cwd: str = "") -> str:
    ...
    if target is None:
        return (
            f"Working directory '{cwd}' is outside the selected project or "
            "does not exist. Use '.' or a path inside the selected folder."
        )
    try:
        proc = subprocess.run(
            command,
